...
class NetworkLstm(tnn.Module):
    ...
    def __init__(self):
        ...
        """
        self.LSTM = torch.nn.LSTM(input_size=50, hidden_size=100, batch_first=True)
    ...
    def forward(self, input, length):
        ...
        x = x.squeeze()
        # x = torch.sigmoid(x)
        # if x.size() == torch.Size([1, 64, 1]) or x.size() == torch.Size([1, 40, 1]) or x.size() == torch.Size([1, 5, 1]):
        #     x = x.view(x.shape[1])
        return x
...
class NetworkCnn(tnn.Module):
    ...
    def __init__(self):
        ...
        self.ln = torch.nn.Linear(in_features=50, out_features=1)
    ...
    def forward(self, input, length):
        ...
        x = self.relu(self.conv1(self.pool(x)))
        x = self.relu(self.conv1(self.pool(x)))
        # print(x.size())
        gpool = torch.nn.MaxPool1d(kernel_size=x.shape[-1])  # kernel size = sequence length
        x = gpool(x)
        # print(x.size())
        x = x.view(x.shape[0], -1)  # transfer inputs to 2-dim
        # print(x.size())
        x = self.ln(x)
        x = x.squeeze()

        # x = torch.sigmoid(x)
        # if x.size() == torch.Size([40, 1]) or x.size() == torch.Size([64, 1]) or x.size() == torch.Size([5, 1]):
        #     x = x.view(x.shape[0])
...
`
    ...
    """
    output_list = outputs.numpy()
    label_list = labels.numpy()
    tp, tn, fp, fn = 0, 0, 0, 0

    for i in range(len(output_list)):
        if (output_list[i] >= 0.5) & (label_list[i] == 1):
            tp += 1
        if (output_list[i] < 0.5) & (label_list[i] == 0):
            tn += 1
        if (output_list[i] < 0.5) & (label_list[i] == 1):
            fn += 1
        if (output_list[i] >= 0.5) & (label_list[i] == 0):
